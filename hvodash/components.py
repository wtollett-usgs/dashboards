...
def Header(children=None, **kwargs):
    return html.Header(html.H1(
        children=[
            Fa('bar-chart'),
            Link(server.config['TITLE'],
                 href=server.config['URL_BASE_PATHNAME'])
        ],
        **kwargs
    ))
...
def Navbar(
    ...
    for i, (path, text) in enumerate(items):
        ...
        # active if we are on the path of this nav item, or if first_root_nav
        # is enabled and applies for this path
        is_active = (current_path == href) or \
                    (first_root_nav and is_first_root_nav)
...
def RefreshButton(children=None, btnid=None):
    return html.Button([
        html.Span(Fa('refresh')),
        ' Refresh'
    ], className='btn', id=btnid)
